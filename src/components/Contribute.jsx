...
const inputStyle = (hasError) => `
  ...
  focus:outline-none focus:ring-0 ${hasError ? 'border-red-500/50' : 'focus:border-lime-500/50 border-white/10'}
  transition-all duration-300
`;

const CustomDropdown = ({ value, onChange, options, placeholder, required, error }) => {
...
const Label = ({ children, helper, required, error }) => (
  <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-1.5 px-1 gap-1">
    ...
    {error && <span className="text-[9px] text-red-400 font-bold animate-pulse">Required</span>}
    {!error && helper && (
      ...
      {options.map((opt) => (
        <label key={opt} className="flex items-center gap-2 cursor-pointer group select-none bg-[#0B1221] border border-white/5 rounded-lg px-3 py-2 hover:border-white/20 transition-colors">
          <div
            ...
            className={`w-4 h-4 rounded border transition-all flex items-center justify-center shrink-0 ${selectedValues.includes(opt) ? "bg-lime-500 border-lime-500" : "bg-transparent border-white/20"}`}
          >
...
const Contribute = () => {
  ...
  const initialData = {
    ...
    constraints: "", targetUsers: [], appType: "", sensitivity: "Low", priority: "Low",
    coordinatorName: "", coordinatorDesignation: "", coordinatorEmail: "", coordinatorContact: "", consent: false,
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const requiredFields = [
      formData.coordinatorName, formData.coordinatorDesignation,
      formData.coordinatorEmail, formData.coordinatorContact,
      formData.dept, formData.title, formData.consent
    ];
        ...
        <AnimatePresence>
          {!showForm ? (
            <motion.div key="hero" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="flex flex-col items-center text-center space-y-6 md:space-y-10 py-10 md:py-16">
              <div className="px-4 py-1.5 rounded-full border border-lime-500/30 bg-lime-500/5 text-lime-400 text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em]">Faculty & Staff Portal</div>
              <h1 className="text-sm sm:text-xl md:text-3xl lg:text-4xl font-black text-white tracking-wide">HAVE A CHALLENGE ? <br /><span className="text-lime-400">CONTRIBUTE NOW.</span></h1>
              <p className="max-w-xl text-[10px] sm:text-xs md:text-sm text-slate-400 font-medium">Submit departmental requirements and problem statements. Our engineering teams will develop software solutions for you.</p>
            ...
            <div className="fixed inset-0 z-[150] flex items-center justify-center px-3 sm:px-4">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-5xl mt-20 max-h-[85vh] overflow-y-auto bg-[#020817] border border-white/10 rounded-2xl shadow-2xl">
                ...
                <form onSubmit={handleSubmit} className="p-4 sm:p-8 space-y-6" noValidate>
                  <div className={`p-4 sm:p-5 rounded-2xl bg-lime-500/[0.03] border transition-colors ${showError && (!formData.coordinatorName || !formData.coordinatorEmail) ? 'border-red-500/40' : 'border-lime-500/10'} space-y-4`}>
                    <div className="flex items-center gap-2 text-lime-400 uppercase text-[10px] font-black tracking-widest"><ShieldAlert className="w-4 h-4" /> Coordinator Details <span className="text-red-500">*</span></div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-4">
                        <input className={inputStyle(showError && !formData.coordinatorName)} value={formData.coordinatorName} placeholder="Full Name" onChange={(e) => setFormData({ ...formData, coordinatorName: e.target.value })} />
                        <input className={inputStyle(showError && !formData.coordinatorDesignation)} value={formData.coordinatorDesignation} placeholder="Designation" onChange={(e) => setFormData({ ...formData, coordinatorDesignation: e.target.value })} />
                      </div>
                      <div className="space-y-4">
                        <input className={inputStyle(showError && !formData.coordinatorEmail)} value={formData.coordinatorEmail} placeholder="Official Email ID" type="email" onChange={(e) => setFormData({ ...formData, coordinatorEmail: e.target.value })} />
                        <input className={inputStyle(showError && !formData.coordinatorContact)} value={formData.coordinatorContact} placeholder="Contact Number" type="tel" onChange={(e) => setFormData({ ...formData, coordinatorContact: e.target.value })} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label required error={showError && !formData.dept}>Department Name</Label>
                      <CustomDropdown value={formData.dept} onChange={(val) => setFormData({ ...formData, dept: val })} options={departments} placeholder="Select Department" error={showError} />
                    </div>
                    <div>
                      <Label required error={showError && !formData.title}>Problem Title</Label>
                      <input className={inputStyle(showError && !formData.title)} value={formData.title} placeholder="Name of problem..." onChange={(e) => setFormData({ ...formData, title: e.target.value })} />
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-white/5 relative">
                    <label className="flex items-start gap-3 cursor-pointer group">
                      <div onClick={() => setFormData(prev => ({ ...prev, consent: !prev.consent }))} className={`w-5 h-5 shrink-0 rounded border transition-all flex items-center justify-center mt-0.5 ${formData.consent ? "bg-lime-500 border-lime-500" : (showError && !formData.consent ? "border-red-500 bg-red-500/10" : "border-white/20 group-hover:border-lime-400")}`}>
                        {formData.consent && <Check size={12} className="text-black stroke-[4px]" />}
                      </div>
                      <span className="text-[10px] sm:text-xs text-slate-400 font-bold uppercase tracking-wider">I consent for <span className="text-white">Pilot Deployment</span> within the campus. <span className="text-red-500">*</span></span>
                    </label>

                    <AnimatePresence>
                      {showError && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute -top-12 left-0 right-0 flex justify-center">
                          <div className="bg-red-500/10 border border-red-500/50 backdrop-blur-md px-4 py-2 rounded-full flex items-center gap-2 text-red-400 text-[10px] font-bold uppercase tracking-tighter shadow-xl">
                            <AlertCircle size={14} /> Please fill out all required fields to proceed
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
